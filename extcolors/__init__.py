...
def compress(counter, tolerance):
    ...
    colors = [item[0] for item in sorted(
...
def print_result(counter, total):
    ...
    for key, value in counter:
        print("{0:15}:{1:>7}% ({2})".format(str(key), "{0:.2f}".format(
            (float(value) / float(total)) * 100.0), value))
    print("\nPixels in output: {} of {}".format(
        sum([c[1] for c in counter]), total))
